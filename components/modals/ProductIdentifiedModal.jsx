...
export default function ProductIdentifiedModal({ isOpen, onClose, onSearch, productName, onEdit, userCredits }) {
    ...
    return (<div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-[9999] p-4">
      <div className="bg-[#1a1a1f] border border-white/20 rounded-2xl p-6 sm:p-8 max-w-md w-full shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="text-center mb-4 sm:mb-6">
          <div className={`w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4 ${hasCredits ? 'bg-green-500/20 border-2 border-green-500/50' : 'bg-red-500/20 border-2 border-red-500/50'}`}>
            {hasCredits ? (<CheckCircle className="w-7 h-7 sm:w-8 sm:h-8 text-green-400"/>) : (<XCircle className="w-7 h-7 sm:w-8 sm:h-8 text-red-400"/>)}
          </div>
        ...
        <div className="bg-gradient-to-br from-white/10 to-white/5 rounded-xl p-4 sm:p-5 mb-4 border border-white/20 shadow-lg">
          ...
          <div className="relative group">
            <textarea value={editedName} onChange={(e) => handleEdit(e.target.value)} onFocus={() => setIsEditing(true)} onBlur={() => setIsEditing(false)} disabled={!hasCredits} rows={isLongText ? 4 : 2} className={`w-full px-4 py-3 bg-black/70 border-2 rounded-xl text-white text-sm sm:text-base outline-none transition-all resize-none disabled:opacity-50 ${isEditing
            ? 'border-blue-500 shadow-lg shadow-blue-500/20 ring-2 ring-blue-500/30'
            : 'border-white/30 hover:border-white/50'}`} placeholder="Digite o nome do produto..."/>
            {!isEditing && hasCredits && (<div className="absolute right-3 top-3 pointer-events-none">
        ...
        {hasCredits ? (<>
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
              ...
              <button onClick={onSearch} className="w-full sm:flex-1 px-4 py-2.5 sm:px-6 sm:py-3 bg-gradient-to-r from-blue-500 to-purple-500 rounded-xl text-white font-semibold hover:scale-105 transition-transform shadow-lg text-sm sm:text-base">
                Buscar Menor Preço
              </button>
            </div>
          </>) : (<>
