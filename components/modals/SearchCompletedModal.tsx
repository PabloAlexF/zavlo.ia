...
interface SearchCompletedModalProps {
  isOpen: boolean;
  onClose: () => void;
  creditsUsed: number;
  resultsCount: number;
...
export default function SearchCompletedModal({ 
  isOpen, 
  onClose, 
  creditsUsed,
  resultsCount 
}: SearchCompletedModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white/5 border border-white/10 rounded-2xl p-8 max-w-md w-full">
        ...
        <button
          onClick={onClose}
          className="w-full px-6 py-3 bg-gradient-to-r from-blue-500 to-purple-500 rounded-xl text-white font-semibold hover:scale-105 transition-transform"
        >
          Ver Resultados
        </button>
      </div>
