...
export default function TextSearchModal({ isOpen, onClose, onConfirm, userCredits }: TextSearchModalProps) {
      ...
      <div className="bg-[#1a1a1f] border border-white/20 rounded-2xl p-6 sm:p-8 max-w-md w-full shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="text-center mb-4 sm:mb-6">
          <div className={`w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4 ${
            hasCredits ? 'bg-yellow-500/20 border-2 border-yellow-500/50' : 'bg-red-500/20 border-2 border-red-500/50'
          }`}>
            {hasCredits ? (
              <AlertTriangle className="w-7 h-7 sm:w-8 sm:h-8 text-yellow-400" />
            ) : (
