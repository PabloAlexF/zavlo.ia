...
export function PlanExpirationAlert() {
    ...
    const getAlertColor = () => {
        ...
        if (planStatus.daysLeft && planStatus.daysLeft <= 3)
    ...
    return (<div className={`fixed top-20 left-1/2 -translate-x-1/2 z-50 max-w-2xl w-full mx-4 animate-slide-down`}>
      <div className={`${getAlertColor()} backdrop-blur-sm rounded-xl border p-4 shadow-2xl flex items-start gap-3`}>
        ...
        <div className="flex-1">
          ...
          {planStatus.isExpired ? (<Link href="/plans" className="inline-flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
              Renovar Agora
            </Link>) : (<Link href="/plans" className="inline-flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors border border-white/20">
              Ver Planos
            </Link>)}
        </div>
