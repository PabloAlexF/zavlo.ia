...
export function SearchHub({ onSearch, onImageSearch, initialValue = '' }) {
        ...
        if (file) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    const productName = ((_a = data.results) === null || _a === void 0 ? void 0 : _a.productName) || data.productName || 'Produto não identificado';
    ...
    return (<div className="w-full max-w-2xl mx-auto px-4 relative z-20">
      {/* Mode Toggle */}
      <div className="flex bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-1 mb-8 w-fit mx-auto">
        <button onClick={() => setActiveMode('text')} className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-medium transition-all ${activeMode === 'text'
            ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg shadow-purple-500/20'
            : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
          <Search className="w-4 h-4"/>
          Texto
        </button>
        <button onClick={() => setActiveMode('image')} className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-medium transition-all relative ${activeMode === 'image'
            ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg shadow-purple-500/20'
            : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
      ...
      {/* Content */}
      {activeMode === 'text' ? (<div className="space-y-4">
          {/* Search Input */}
          <form onSubmit={handleSubmit} className="relative group">
            ...
            <div className="relative flex items-center">
              <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="O que você está procurando?" className="w-full h-16 pl-6 pr-16 bg-black/80 backdrop-blur-xl border border-white/10 rounded-2xl text-lg text-white placeholder-gray-500 focus:outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/50 transition-all shadow-2xl"/>
              <button type="submit" className="absolute right-2 p-3 bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl text-white hover:scale-105 transition-transform shadow-lg">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            ...
            ].map((item) => (<button key={item.text} onClick={() => handleQuickSearch(item.query)} className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-full hover:bg-white/10 hover:border-white/20 transition-all group">
        ...
        </div>) : (<div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          {/* AI Upload Area */}
          <div onDrop={handleDrop} onDragOver={(e) => { e.preventDefault(); setDragActive(true); }} onDragLeave={() => setDragActive(false)} className={`relative border-2 border-dashed rounded-3xl p-8 text-center transition-all ${dragActive
            ...
            <div className="space-y-4">
              {/* AI Icon */}
              <div className={`w-16 h-16 mx-auto rounded-2xl flex items-center justify-center ${getUserCredits() > 0
              ...
              <div>
                <h3 className={`text-lg font-semibold mb-2 ${getUserCredits() > 0 ? 'text-white' : 'text-gray-400'}`}>
              ...
              {/* Upload Button */}
              {getUserCredits() > 0 ? (<label className="block">
                  <input type="file" accept="image/*" onChange={handleFileInput} className="hidden" disabled={isIdentifying}/>
                  <div className={`w-full h-12 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-2xl flex items-center justify-center font-medium cursor-pointer hover:scale-105 transition-transform shadow-lg ${isIdentifying ? 'opacity-50 cursor-not-allowed' : ''}`}>
                    {isIdentifying ? (<>
                ...
                </label>) : (<button onClick={() => router.push('/plans')} className="w-full h-12 bg-gradient-to-r from-orange-600 to-red-600 text-white rounded-2xl flex items-center justify-center font-medium cursor-pointer hover:scale-105 transition-transform shadow-lg">
          ...
          <div className="space-y-3">
            ...
            ].map((feature, i) => (<div key={i} className="flex items-center gap-3 p-3 bg-white/5 rounded-xl border border-white/10">
      ...
      {/* Modais */}
      <TextSearchModal isOpen={showTextModal} onClose={() => setShowTextModal(false)} onConfirm={confirmTextSearch} userCredits={getUserCredits()}/>

      <ProductIdentifiedModal isOpen={showImageModal} onClose={() => setShowImageModal(false)} onSearch={confirmImageSearch} productName={identifiedProduct} onEdit={(name) => setIdentifiedProduct(name)} userCredits={getUserCredits()}/>
    </div>);
}
