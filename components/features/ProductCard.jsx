    ...
    return (<Link href={`/product/${product.id}`}>
      <div className="group cursor-pointer transform transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-purple-500/20">
        <div className="relative w-full bg-gradient-to-br from-white/5 to-white/[0.02] rounded-2xl overflow-hidden mb-4 shadow-lg border border-white/10" style={{ height: '280px' }}>
          {hasImage && !imageError ? (<>
              {/* Loading Skeleton */}
              {isLoading && (<div className="absolute inset-0 bg-white/10 animate-pulse z-10"/>)}

              {/* Image */}
              <img src={currentImageUrl || ''} alt={product.title} style={{
                width: '100%',
                height: '100%',
                objectFit: 'cover',
                display: 'block',
            ...
            }} onError={handleImageError} onLoad={handleImageLoad} loading="lazy"/>

              {/* Navigation Buttons */}
              {hasMultipleImages && !isLoading && (<>
                  <button onClick={prevImage} aria-label="Imagem anterior" style={{
                    position: 'absolute',
                    left: '8px',
                    top: '50%',
                  ...
                  <button onClick={nextImage} aria-label="Próxima imagem" style={{
                    position: 'absolute',
                    right: '8px',
                    top: '50%',
                    transform: 'translateY(-50%)',
                  ...
                  <div style={{
                    ...
                    {images.map((_, index) => (<div key={index} className={`h-1.5 rounded-full transition-all ${index === currentImageIndex ? 'bg-white w-4' : 'bg-white/50 w-1.5'}`}/>))}
                  </div>
                </>)}
              ...
              <button onClick={toggleFavorite} style={{
                ...
                left: '12px',
                zIndex: 20
            }} className={`w-9 h-9 rounded-full backdrop-blur-md flex items-center justify-center transition-all ${isFavorite
                ? 'bg-red-500/90 scale-110'
                : 'bg-black/40 hover:bg-black/60'}`} aria-label="Favoritar">
