import { BRAND_SET, GENERIC_PRODUCTS } from './constants';
import { extractProductInfo } from './queryProcessor';
...
function findClosestBrand(word) {
    ...
    for (const brand of BRAND_SET) {
        ...
        if (Math.abs(brand.charCodeAt(0) - normalized.charCodeAt(0)) > 2)
            continue;
...
export function buildSearchQuery(parsed, condition, location, gender, priceMax, storage, sortBy) {
