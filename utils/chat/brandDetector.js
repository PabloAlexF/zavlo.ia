 ...
 */
export function normalizeProductAliases(query) {
 ...
 */
export function detectProductVersion(query) {
 ...
 */
export function detectModelAndBrand(query) {
    const normalized = query.toLowerCase();
 ...
 */
export function enrichProductQuery(query) {
    // 0. Normaliza aliases
...
export function optimizeQueryOrder(parts) {
    ...
    if (parts.attributes && parts.attributes.length > 0) {
        // Adiciona atributos na ordem original (já vem correto do queryProcessor)
 ...
 */
export function generateSmartBrandQuestion(detectedBrand, originalQuery) {
