...
export class GoogleLensService {
  ...
  private readonly actorId = 'borderline~google-lens';
   ...
   */
  private async uploadBase64ToCloudinary(base64Data: string): Promise<string> {
   ...
   */
  async identifyProduct(imageUrl: string): Promise<{ productName: string; confidence: number; results?: any[] }> {
    try {
      this.logger.log(`Identificando produto na imagem: ${imageUrl?.substring(0, 100)}...`);
      ...
      this.logger.log(`🔍 Usando URL final para Google Lens: ${finalImageUrl?.substring(0, 100)}...`);
      ...
      for (let i = 0; i < maxAttempts; i++) {
        ...
        if (statusData.data.status === 'SUCCEEDED') {
          ...
          this.logger.log(`📦 Dataset items: ${finalResults?.length || 0}`);
          break;
