  ...
  const plans: Plan[] = [
    {
      ...
      price: 0,
      yearlyPrice: 0,
      features: ['1 comparação por texto', '3 lojas (OLX, ML, Amazon)', '10 resultados por busca', 'Com anúncios'],
      cta: 'Testar Agora',
  ...
  const handleSelectPlan = async (planName: string): Promise<void> => {
    ...
    if (planName === 'Gratuito') {
      try {
        ...
        const response = await fetch('http://localhost:3001/api/v1/users/plan', {
          method: 'POST',
          headers: {
