...
export default function MarketplacePage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (response.ok) {
          ...
          const zavloListings = listings.filter((product: any) => 
            (product.source === 'Zavlo.ia' || product.marketplace === 'Zavlo.ia' || product.source === 'zavlo') &&
        ...
        <main className="max-w-7xl mx-auto px-4">
          <div className="animate-pulse space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {[...Array(8)].map((_, i) => (
                <div key={i} className="bg-white/5 rounded-2xl h-96" />
              ))}
            </div>
          </div>
        </main>
  ...
  return (
    <div className="min-h-screen bg-[#0B0B0F] pt-20 pb-12 relative overflow-hidden">
      {/* Background Effects */}
      ...
      <main className="max-w-7xl mx-auto px-4 relative z-10">
        ...
        <motion.div 
          className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/20 rounded-2xl p-6 mb-6 shadow-2xl"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 bg-black/50 border border-white/20 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/50 transition"
              />
            </div>
            ...
            <button
              ...
              className="relative px-6 py-3 bg-white/5 border border-white/10 rounded-xl text-white font-semibold hover:bg-white/10 transition-all flex items-center gap-2"
            >
              ...
              >
                <div className="pt-6 mt-6 border-t border-white/10 grid md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-4 py-2.5 bg-black/50 border border-white/20 rounded-xl text-white focus:outline-none focus:border-blue-500 cursor-pointer"
                      style={{ colorScheme: 'dark' }}
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 bg-black/50 border border-white/20 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-blue-500"
                    />
                  </div>
                ...
                {activeFiltersCount > 0 && (
                  <button
                    ...
                    className="mt-4 px-4 py-2 bg-red-500/20 text-red-400 rounded-lg font-semibold hover:bg-red-500/30 transition-all flex items-center gap-2"
                  >
                    <X className="w-4 h-4" />
                    Limpar Filtros
                  </button>
        ...
        {activeFiltersCount > 0 && (
          ...
          >
            {searchQuery && (
              <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-blue-500/20 text-blue-300 rounded-full text-sm font-medium">
            ...
            {selectedCategory !== 'all' && (
              <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-purple-500/20 text-purple-300 rounded-full text-sm font-medium">
                {categories.find(c => c.id === selectedCategory)?.name}
                <X className="w-4 h-4 cursor-pointer hover:text-white" onClick={() => setSelectedCategory('all')} />
              </span>
            )}
            {conditionFilter !== 'all' && (
              <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-green-500/20 text-green-300 rounded-full text-sm font-medium">
          ...
          >
            {filteredListings.map((listing, index) => (
              <motion.div
                ...
                className={`bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden hover:border-blue-500/50 transition-all group ${
                    ...
                    {listing.location && (
                      <span className="flex items-center gap-1">
                        ...
                        {typeof listing.location === 'string' 
                          ? listing.location 
                          : `${(listing.location as any).city || ''}, ${(listing.location as any).state || ''}`
                        }
                      </span>
                    )}
                  </div>
