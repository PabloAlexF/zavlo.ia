...
export default function ProductDetails() {
    ...
    const location = typeof product.location === 'string' ? product.location : `${((_a = product.location) === null || _a === void 0 ? void 0 : _a.city) || ''}, ${((_b = product.location) === null || _b === void 0 ? void 0 : _b.state) || ''}`;
    ...
    useEffect(() => {
        const checkFavorite = async () => {
            console.log('[PRODUCT] Verificando se produto está favoritado:', product === null || product === void 0 ? void 0 : product.id);
            const user = localStorage.getItem('zavlo_user');
            ...
            if (!user) {
                ...
                const isFav = favorites.includes(product === null || product === void 0 ? void 0 : product.id);
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    const isFav = favorites.some((f) => f.productId === (product === null || product === void 0 ? void 0 : product.id));
    ...
    return (<div className="min-h-screen bg-white text-black">
      ...
      <div className="max-w-7xl mx-auto px-4 pt-20 pb-16">
        {/* Back Button */}
        <button onClick={() => router.back()} className="text-sm text-gray-600 mb-6 hover:text-black transition flex items-center gap-2">
          ← Voltar
        </button>
        ...
        <div className="space-y-6">
          ...
          <div className="relative aspect-square bg-gray-50 rounded-2xl overflow-hidden">
            {hasImage && !imageError ? (<>
                ...
                {/* Image */}
                <img src={currentImageUrl || ''} alt={product.title} className="w-full h-full object-cover" style={{ opacity: isLoading ? 0.5 : 1 }} onError={handleImageError} onLoad={handleImageLoad} loading="lazy"/>

                {/* Navigation Buttons - Smaller on mobile */}
                {hasMultipleImages && !isLoading && (<>
                    <button onClick={prevImage} className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 md:w-10 md:h-10 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-white transition z-20">
                      <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7"/>
                      </svg>
                    </button>
                    
                    <button onClick={nextImage} className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 md:w-10 md:h-10 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-white transition z-20">
                      <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 z-20">
                      {product.images.map((_, index) => (<div key={index} className={`h-1.5 rounded-full transition-all ${index === currentImageIndex ? 'bg-white w-4' : 'bg-white/50 w-1.5'}`}/>))}
                    </div>
                  </>)}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 rounded-xl p-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center py-2">
                  ...
                  <span className="font-medium text-sm">{typeof product.seller === 'string' ? product.seller : ((_c = product.seller) === null || _c === void 0 ? void 0 : _c.name) || 'N/A'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="sticky bottom-4 z-30 flex gap-3">
              <button onClick={toggleFavorite} className={`px-6 py-4 rounded-xl font-semibold transition shadow-lg flex items-center justify-center gap-2 ${isFavorite
