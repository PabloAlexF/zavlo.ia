...
import { Search, X, Eye, MousePointerClick, Edit, Trash2, Power, ExternalLink, Plus, PackageOpen } from 'lucide-react';
export default function MyListingsPage() {
    ...
    const [deleteModal, setDeleteModal] = useState({ show: false, listingId: '', title: '' });
    useEffect(() => {
        loadListings();
    }, []);
    useEffect(() => {
    ...
    const filterListings = () => {
        ...
        if (statusFilter !== 'all') {
            filtered = filtered.filter(listing => statusFilter === 'active' ? listing.active : !listing.active);
        }
    ...
    const toggleActive = async (listingId) => {
        const user = localStorage.getItem('zavlo_user');
        if (!user)
            return;
        try {
            ...
            if (response.ok) {
                setListings(prev => prev.map(listing => listing.id === listingId ? Object.assign(Object.assign({}, listing), { active: !listing.active }) : listing));
    ...
    return (<div className="min-h-screen bg-[#0B0B0F] relative overflow-hidden">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 pt-24 pb-12">
        <motion.div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div>
          ...
          <motion.button onClick={() => router.push('/sell')} className="w-full sm:w-auto bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-3 rounded-xl font-semibold shadow-lg hover:shadow-blue-500/50 transition-all flex items-center justify-center gap-2" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
        ...
        {/* Search and Filters */}
        <motion.div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 mb-8 shadow-2xl" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <div className="space-y-3">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-gray-400"/>
              <input type="text" placeholder="Buscar anúncios..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-black/50 border border-white/20 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/50 transition"/>
            </div>

            <div className="grid grid-cols-2 gap-3">
              <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-4 py-3 bg-black/50 border border-white/20 rounded-xl text-white focus:outline-none focus:border-blue-500 cursor-pointer" style={{ colorScheme: 'dark' }}>
              ...
              <select value={conditionFilter} onChange={(e) => setConditionFilter(e.target.value)} className="px-4 py-3 bg-black/50 border border-white/20 rounded-xl text-white focus:outline-none focus:border-blue-500 cursor-pointer" style={{ colorScheme: 'dark' }}>
          ...
          {(searchQuery || statusFilter !== 'all' || conditionFilter !== 'all') && (<div className="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-white/10">
              {searchQuery && (<span className="inline-flex items-center gap-1 px-2.5 py-1 bg-blue-500/20 text-blue-300 rounded-full text-xs">
                  "{searchQuery.slice(0, 15)}{searchQuery.length > 15 ? '...' : ''}"
                  <X className="w-3 h-3 cursor-pointer" onClick={() => setSearchQuery('')}/>
                </span>)}
              {statusFilter !== 'all' && (<span className="inline-flex items-center gap-1 px-2.5 py-1 bg-green-500/20 text-green-300 rounded-full text-xs">
                  {statusFilter === 'active' ? 'Ativos' : 'Inativos'}
                  <X className="w-3 h-3 cursor-pointer" onClick={() => setStatusFilter('all')}/>
                </span>)}
              {conditionFilter !== 'all' && (<span className="inline-flex items-center gap-1 px-2.5 py-1 bg-purple-500/20 text-purple-300 rounded-full text-xs">
        ...
        {filteredListings.length === 0 ? (<div className="text-center py-12 px-4">
            ...
            {listings.length === 0 ? (<button onClick={() => router.push('/sell')} className="bg-blue-600 active:bg-blue-700 text-white px-6 py-2.5 rounded-lg font-medium transition-colors">
                Criar Anúncio
              </button>) : (<button onClick={clearFilters} className="bg-white/10 active:bg-white/20 text-white px-6 py-2.5 rounded-lg font-medium transition-colors">
                Limpar Filtros
              </button>)}
          </div>) : (<motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
            {filteredListings.map((listing, index) => (<motion.div key={listing.id} className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden hover:border-blue-500/50 transition-all group" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + index * 0.05 }} whileHover={{ y: -4, scale: 1.02 }}>
                {/* Image Section */}
                <div className="relative aspect-square w-full bg-white/5">
                  {listing.images && listing.images.length > 0 ? (<img src={listing.images[0]} alt={listing.title} className="w-full h-full object-cover" onError={(e) => {
      ...
      {deleteModal.show && (<div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#1a1a2e] border border-white/10 rounded-2xl p-6 max-w-md w-full">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteModal({ show: false, listingId: '', title: '' })} className="flex-1 bg-white/10 hover:bg-white/20 text-white py-3 rounded-lg font-medium transition-colors">
                Cancelar
              </button>
              <button onClick={() => deleteListing(deleteModal.listingId)} className="flex-1 bg-red-600 hover:bg-red-700 text-white py-3 rounded-lg font-medium transition-colors">
                Excluir
              </button>
            </div>
          </div>
