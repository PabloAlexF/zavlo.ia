...
import { CreditCard, QrCode, Check, ArrowLeft, Lock, Zap } from 'lucide-react';
...
function CreditsCheckoutContent() {
    ...
    const paymentMethods = [
        { id: 'pix', Icon: QrCode, title: 'PIX', subtitle: 'Aprovação instantânea', color: 'from-green-500 to-green-600' },
        { id: 'checkout', Icon: CreditCard, title: 'Cartão ou Boleto', subtitle: 'Crédito, Débito ou Boleto', color: 'from-blue-500 to-blue-600' }
    ];
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {paymentMethods.map((method) => (<button key={method.id} onClick={() => setPaymentMethod(method.id)} className={`relative p-6 rounded-xl border-2 transition-all hover:scale-105 ${paymentMethod === method.id ? 'border-blue-500 bg-blue-500/10' : 'border-white/10 bg-white/5'}`}>
            ...
            {pixData && (<div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 text-center">
                ...
                <button onClick={() => { navigator.clipboard.writeText(pixData.qr_code); setModal({ type: 'success', title: 'Copiado!', message: 'Cole no app de pagamentos.' }); }} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 mb-6">Copiar Código PIX</button>
            ...
            }} disabled={confirmingPayment} className="w-full py-4 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl font-bold">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
              ...
              <button onClick={handlePayment} disabled={processing || !!pixData} className="w-full py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl font-bold hover:scale-105 transition-all disabled:opacity-50 mb-6">
...
export default function CreditsCheckout() {
    return (<Suspense fallback={<div className="min-h-screen bg-[#0B0B0F] flex items-center justify-center"><div className="text-white">Carregando...</div></div>}>
