...
import { CreditCard, QrCode, Shield, Check, ArrowLeft, Sparkles, Lock } from 'lucide-react';
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {paymentMethods.map((method) => (<button key={method.id} onClick={() => setPaymentMethod(method.id)} className={`relative p-6 rounded-xl border-2 transition-all hover:scale-105 ${paymentMethod === method.id
                ? 'border-blue-500 bg-blue-500/10'
                : 'border-white/10 bg-white/5 hover:border-white/20'}`}>
                      {paymentMethod === method.id && (<div className="absolute -top-2 -right-2 w-6 h-6 bg-blue-500 rounded-full flex items-center justify-center">
              ...
              {/* Payment Form */}
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
                {paymentMethod === 'checkout' && (<div className="text-center py-12">
                ...
                {paymentMethod === 'pix' && (<div className="text-center py-12">
                    {pixData ? (<>
                    ...
                    try {
                        ...
                        if (response.ok && data.status === 'approved') {
                            ...
                            if (user) {
                                const userData = JSON.parse(user);
                                const updatedUser = Object.assign(Object.assign({}, userData), { credits: (userData.credits || 0) + (data.credits || 15), plan: data.plan || 'basic' });
                ...
                }} disabled={confirmingPayment} className="w-full max-w-md mx-auto py-4 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl font-bold hover:scale-105 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-green-500/20 flex items-center justify-center gap-2">
                          {confirmingPayment ? (<>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 sticky top-24">
                ...
                <div className="space-y-4 mb-6">
                  ...
                  {cycle === 'yearly' && (<div className="flex items-center gap-2 text-xs text-green-400 bg-green-500/10 px-3 py-2 rounded-lg">
                ...
                <button onClick={handlePayment} disabled={processing} className="w-full py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl font-bold hover:scale-105 transition-all disabled:opacity-50 disabled:cursor-not-allowed mb-6 shadow-lg shadow-blue-500/20">
