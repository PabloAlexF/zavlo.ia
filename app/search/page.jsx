...
function SearchContent() {
    ...
    const getSortedAndFilteredResults = (products) => {
    ...
    const performSearch = async () => {
        ...
        try {
            ...
            const response = await fetch(endpoint, {
                method: 'POST',
                headers: Object.assign({ 'Content-Type': 'application/json' }, ((userData === null || userData === void 0 ? void 0 : userData.token) && { 'Authorization': `Bearer ${userData.token}` })),
    ...
    if (!query || query.trim() === '') {
        return (<div className="min-h-screen bg-[#0B0B0F] pt-24 pb-12 relative overflow-hidden">
        ...
        <main className="max-w-5xl mx-auto px-4 relative z-10">
          ...
          {/* Chat IA Explanation */}
          <motion.div className="bg-gradient-to-br from-blue-500/10 via-purple-500/10 to-pink-500/10 border border-blue-500/20 rounded-3xl p-8 mb-8 backdrop-blur-sm" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            <motion.div className="flex items-start gap-4 mb-6" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}>
              <motion.div className="w-14 h-14 bg-gradient-to-br from-blue-500 to-purple-600 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-lg" whileHover={{ scale: 1.1, rotate: 5 }} transition={{ type: "spring", stiffness: 300 }}>
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-6">
              <motion.div className="bg-black/30 rounded-2xl p-6 border border-white/10" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }} whileHover={{ scale: 1.02, borderColor: "rgba(255,255,255,0.2)" }}>
                <h3 className="text-lg font-bold text-white mb-3 flex items-center gap-2">
                  <motion.div className="w-8 h-8 bg-green-500/20 rounded-lg flex items-center justify-center" whileHover={{ scale: 1.2, rotate: 360 }} transition={{ duration: 0.3 }}>
                    <CheckCircle className="w-5 h-5 text-green-400"/>
                  </motion.div>
                  Como funciona?
                </h3>
                <ul className="space-y-2 text-gray-300">
                  <motion.li className="flex items-start gap-2" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }}>
                    <span className="text-blue-400 mt-1">•</span>
                    <span><strong className="text-white">Converse naturalmente:</strong> "Quero um notebook para programar até R$ 3000"</span>
                  </motion.li>
                  <motion.li className="flex items-start gap-2" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }}>
                    <span className="text-purple-400 mt-1">•</span>
                    <span><strong className="text-white">IA entende contexto:</strong> Ela faz perguntas para refinar sua busca</span>
                  </motion.li>
                  <motion.li className="flex items-start gap-2" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7 }}>
              ...
              <motion.div className="bg-black/30 rounded-2xl p-6 border border-white/10" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }} whileHover={{ scale: 1.02, borderColor: "rgba(255,255,255,0.2)" }}>
                <h3 className="text-lg font-bold text-white mb-3 flex items-center gap-2">
                  <motion.div className="w-8 h-8 bg-yellow-500/20 rounded-lg flex items-center justify-center" whileHover={{ scale: 1.2, rotate: -360 }} transition={{ duration: 0.3 }}>
            ...
            ].map((example, i) => (<motion.li key={i} className="bg-white/5 rounded-lg p-2 border border-white/10 flex items-center gap-2" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 + i * 0.1 }} whileHover={{ scale: 1.02, backgroundColor: "rgba(255,255,255,0.1)" }}>
            ...
            <motion.div className="bg-gradient-to-r from-orange-500/20 to-red-500/20 border border-orange-500/30 rounded-2xl p-6 mb-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }} whileHover={{ scale: 1.01 }}>
              <div className="flex items-start gap-3">
                <motion.div className="w-10 h-10 bg-orange-500/30 rounded-xl flex items-center justify-center flex-shrink-0" animate={{
                scale: [1, 1.1, 1],
                rotate: [0, 5, -5, 0]
            }} transition={{
                duration: 2,
            ...
            ].map((benefit, i) => (<motion.li key={i} className="flex items-center gap-2" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.9 + i * 0.1 }}>
            ...
            }} className="px-8 py-4 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 rounded-xl text-white font-bold text-lg shadow-lg hover:shadow-blue-500/50 transition-all flex items-center justify-center gap-2" whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}>
                <MessageSquare className="w-5 h-5"/>
                Conversar com IA
              </motion.button>
              
              <motion.button onClick={() => router.push('/auth')} className="px-8 py-4 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl text-white font-semibold text-lg transition-all flex items-center justify-center gap-2" whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}>
    ...
    return (<div className="min-h-screen bg-[#0B0B0F] pt-16 sm:pt-20 pb-8 sm:pb-12 relative overflow-hidden">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        <div className="mb-6 sm:mb-8">
          <form onSubmit={handleNewSearch} className="relative group">
            ...
            <div className="relative flex items-center">
              ...
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Buscar com IA..." className="w-full h-12 sm:h-14 pl-12 sm:pl-16 pr-20 sm:pr-32 bg-black/90 backdrop-blur-xl border border-white/20 rounded-2xl text-white text-base sm:text-lg placeholder-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/50 outline-none transition-all shadow-2xl"/>
        ...
        <div className="lg:hidden mb-4">
          <button onClick={() => setShowMobileFilters(!showMobileFilters)} className="w-full flex items-center justify-between px-4 py-3 bg-gradient-to-r from-white/10 to-white/5 border border-white/20 rounded-xl text-white font-semibold hover:from-white/15 hover:to-white/10 transition-all">
          ...
          <div className={`lg:col-span-1 ${showMobileFilters ? 'block' : 'hidden'} lg:block`}>
            <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/20 rounded-2xl p-4 sm:p-6 lg:sticky lg:top-24 shadow-2xl">
              <div className="flex items-center justify-between mb-4 sm:mb-6">
                ...
                <button onClick={() => setShowMobileFilters(false)} className="lg:hidden p-1 text-gray-400 hover:text-white transition-colors">
                  <X className="w-5 h-5"/>
                </button>
              </div>
...
export default function SearchPage() {
    return (<Suspense fallback={<div className="min-h-screen bg-[#0B0B0F] flex items-center justify-center">
