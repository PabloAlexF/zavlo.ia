...
import { Send, Sparkles, Loader2, X, Coins, Menu, Plus, MessageSquare, Trash2, ChevronLeft, MapPin, Trophy, Flag, Bot, HelpCircle, Hash, Search, Globe, Zap, CheckCircle2, CreditCard, ArrowRight, Image as ImageIcon, ChevronDown, Smartphone, Headphones, Laptop, Tv, Shirt, Car } from 'lucide-react';
...
export default function ChatPage() {
    ...
    const loadChatHistory = () => {
        try {
            ...
            if (saved) {
                const history = JSON.parse(saved).map((chat) => (Object.assign(Object.assign({}, chat), { createdAt: new Date(chat.createdAt), updatedAt: new Date(chat.updatedAt), messages: chat.messages.map((m) => (Object.assign(Object.assign({}, m), { timestamp: new Date(m.timestamp) }))) })));
    ...
    const saveChatToHistory = () => {
        setChatHistory(prevHistory => {
            ...
            try {
                ...
                const chatTitle = ((_a = messages.find(m => m.type === 'user')) === null || _a === void 0 ? void 0 : _a.content.slice(0, 30)) || 'Nova conversa';
                const existingIndex = prevHistory.findIndex(c => c.id === currentChatId);
                const chatData = {
                    id: currentChatId,
                    title: chatTitle,
                    messages: messages.slice(-50).map(m => { var _a; return m.type === 'products' ? Object.assign(Object.assign({}, m), { products: (_a = m.products) === null || _a === void 0 ? void 0 : _a.slice(0, 6) }) : m; }),
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (typeof data.remainingCredits === 'number') {
                    ...
                    const updatedUser = Object.assign(Object.assign({}, userData), { credits: data.remainingCredits });
                    localStorage.setItem('zavlo_user', JSON.stringify(updatedUser));
                    window.dispatchEvent(new Event('userChanged'));
                }
                const creditsUsed = data.creditsUsed || 1;
                const remainingCredits = (_a = data.remainingCredits) !== null && _a !== void 0 ? _a : userCredits - 1;
    ...
    const executeImageSearch = async (sortBy) => {
        var _a;
        if (!detectedProductName || loading)
            return;
        setLoading(true);
    ...
    const handleSend = async (messageText, isInternal = false) => {
        ...
        if (!currentInput || !String(currentInput).trim() || loading)
            return;
        ...
        if (!isInternal && chatState === 'idle' && !pendingSearch) {
            const contextChange = detectContextChange(currentInput, contextManager.get().conversationHistory);
            ...
            if (contextChange.hasChange && (contextChange.type === 'correction' || contextChange.type === 'new_search')) {
                ...
                const parts = [];
                if (contextChange.newProduct)
                    parts.push(contextChange.newProduct);
                if (contextChange.newBrand)
                    parts.push(contextChange.newBrand);
                if (contextChange.newCondition)
                    parts.push(contextChange.newCondition);
                if (contextChange.newLocation)
                    parts.push(contextChange.newLocation);
        ...
        if (chatState === 'awaiting_location') {
            ...
            setPendingSearch(prev => {
                if (!prev)
                    return prev;
                return Object.assign(Object.assign({}, prev), { location: updatedLocation });
            });
            // Se tem categoria, gerar busca final com respostas
            if (pendingSearch === null || pendingSearch === void 0 ? void 0 : pendingSearch.category) {
                const allAnswers = Object.assign({}, categoryAnswers);
        ...
        if (chatState === 'category_questions') {
            ...
            const remainingQuestions = filterQuestions(categoryData.questions, Object.assign(Object.assign({}, categoryAnswers), providedInfo));
            ...
            // Salva resposta
            const newAnswers = Object.assign(Object.assign({}, categoryAnswers), { [currentQuestion.id]: parsedAnswer });
        ...
        if (chatState === 'awaiting_condition') {
            ...
            setPendingSearch(prev => {
                if (!prev)
                    return prev;
                return Object.assign(Object.assign({}, prev), { condition });
            });
        ...
        setTimeout(() => {
            ...
            if (parsed.needsLocation) {
                setPendingSearch(prev => {
                    ...
                    return Object.assign(Object.assign({}, prev), { query: cleaned });
                });
                const aiMessage = {
                    id: crypto.randomUUID(),
                    type: 'ai',
            ...
            if (parsed.condition) {
                ...
                setPendingSearch(prev => {
                    if (!prev)
                        return { query: cleaned, condition: parsed.condition };
                    return Object.assign(Object.assign({}, prev), { query: cleaned, condition: parsed.condition });
                });
    ...
    const buildFinalQuery = () => {
        if (!pendingSearch)
            return { query: '', sortBy: 'RELEVANCE' };
        // Extrai priceMax, storage e sortBy com optional chaining
        const priceMax = categoryAnswers === null || categoryAnswers === void 0 ? void 0 : categoryAnswers.price_max;
        const storage = categoryAnswers === null || categoryAnswers === void 0 ? void 0 : categoryAnswers.storage;
        const sortBy = categoryAnswers === null || categoryAnswers === void 0 ? void 0 : categoryAnswers.sort_by;
        return buildSearchQuery(parseProductQuery(pendingSearch.query), pendingSearch.condition, pendingSearch.location, undefined, // gender
        priceMax, storage, sortBy);
    };
    const buildCategoryQuery = (baseQuery, answers, location) => {
        // Parse do produto
        const parsed = parseProductQuery(baseQuery);
        // Extrai dados das respostas
        const condition = answers === null || answers === void 0 ? void 0 : answers.condition;
        const priceMax = answers === null || answers === void 0 ? void 0 : answers.price_max;
        const storage = answers === null || answers === void 0 ? void 0 : answers.storage;
        const gender = answers === null || answers === void 0 ? void 0 : answers.gender;
        const sortBy = answers === null || answers === void 0 ? void 0 : answers.sort_by;
        ...
        const result = buildSearchQuery(parsed, condition, location, gender, priceMax, storage, sortBy);
        console.log('🎯 Query final gerada:', result);
        return result;
    };
    const handleConfirmSearch = async () => {
        var _a;
        if (!pendingSearch || chatState !== 'awaiting_confirmation')
            return;
    ...
    return (<div className="h-screen bg-gradient-to-br from-[#0A0A0F] via-[#0F0F14] to-[#0A0A0F] flex">
      {/* Sidebar */}
      <AnimatePresence>
        {sidebarOpen && (<motion.aside initial={{ x: -280, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -280, opacity: 0 }} transition={{ duration: 0.3, ease: [0.4, 0, 0.2, 1] }} className="w-72 h-full border-r border-white/5 bg-black/40 backdrop-blur-2xl flex flex-col fixed md:relative z-50">
            {/* New Chat Button */}
            <div className="p-4 border-b border-white/5">
              <motion.button onClick={createNewChat} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-blue-500/10 to-purple-500/10 hover:from-blue-500/20 hover:to-purple-500/20 border border-white/10 rounded-2xl text-white transition-all text-sm font-medium shadow-lg shadow-blue-500/5">
            ...
            <div className="flex-1 overflow-y-auto px-3 py-2">
              {chatHistory.length === 0 ? (<div className="text-center py-16 px-4">
                  ...
                  {chatHistory.map((chat) => (<motion.div key={chat.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} whileHover={{ x: 2 }} className={`group relative p-3 rounded-xl transition-all cursor-pointer ${currentChatId === chat.id
                        ? 'bg-gradient-to-r from-blue-500/10 to-purple-500/10 border border-blue-500/20'
                        : 'hover:bg-white/5 border border-transparent'}`} onClick={() => loadChat(chat.id)}>
                      <div className="flex items-start gap-3">
                        <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${currentChatId === chat.id ? 'bg-blue-400' : 'bg-gray-600'}`}/>
                        <p className="text-sm text-gray-300 truncate flex-1 pr-8">{chat.title}</p>
                        <button onClick={(e) => deleteChat(chat.id, e)} className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 hover:bg-red-500/20 rounded-lg transition opacity-100 sm:opacity-0 sm:group-hover:opacity-100" aria-label="Deletar conversa">
            ...
            <div className="p-4 border-t border-white/5 md:hidden">
              <button onClick={() => setSidebarOpen(false)} className="w-full flex items-center justify-center gap-2 px-3 py-2.5 bg-white/5 hover:bg-white/10 rounded-xl text-gray-400 text-sm transition-colors">
      ...
      {/* Mobile Overlay */}
      <AnimatePresence>
        {sidebarOpen && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-40 md:hidden" onClick={() => setSidebarOpen(false)}/>)}
      </AnimatePresence>
      ...
      <div className="flex-1 flex flex-col h-full">
        ...
        <div className="border-b border-white/5 bg-black/20 backdrop-blur-2xl px-4 sm:px-6 h-16 sm:h-18 flex items-center justify-between">
          <div className="flex items-center gap-3 sm:gap-4 min-w-0">
            <motion.button onClick={() => setSidebarOpen(!sidebarOpen)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="p-2 hover:bg-white/10 rounded-xl flex-shrink-0 transition-colors">
              <Menu className="w-5 h-5 text-gray-400"/>
            </motion.button>
            <div className="flex items-center gap-3 min-w-0">
              <Image src="/assets/icons/logo.ico" alt="Zavlo AI" width={36} height={36} className="rounded-2xl flex-shrink-0 shadow-lg shadow-purple-500/20"/>
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            <motion.div whileHover={{ scale: 1.05 }} className="flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border border-yellow-500/20 rounded-xl shadow-lg shadow-yellow-500/5">
              <Coins className="w-4 h-4 text-yellow-400"/>
              <span className="text-sm font-semibold text-white">{userCredits}</span>
            </motion.div>
            <motion.button onClick={() => router.push('/')} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="p-2 hover:bg-white/10 rounded-xl transition-colors">
        ...
        <div className="flex-1 overflow-y-auto px-3 sm:px-4 py-4 sm:py-6">
          <div className="max-w-4xl mx-auto space-y-3 sm:space-y-4">
            {messages.length === 1 && messages[0].type === 'ai' && (<div className="flex flex-col items-center gap-3 sm:gap-4 mt-4 sm:mt-8">
            ...
            ].map((suggestion, i) => (<motion.button key={i} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => {
                    var _a;
                ...
                <div className="w-full max-w-2xl">
                  <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} onClick={() => setShowMoreSuggestions(!showMoreSuggestions)} className="w-full flex items-center justify-center gap-2 px-4 py-3 sm:py-3.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-white transition-colors">
                    <span className="text-sm sm:text-base font-medium">Mais sugestões</span>
                    <motion.div animate={{ rotate: showMoreSuggestions ? 180 : 0 }} transition={{ duration: 0.2 }}>
                      <ChevronDown className="w-5 h-5"/>
                    </motion.div>
                  </motion.button>

                  <AnimatePresence>
                    {showMoreSuggestions && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                ...
                ].map((item, i) => (<button key={i} onClick={() => { var _a; setInput(item.text); setShowMoreSuggestions(false); (_a = inputRef.current) === null || _a === void 0 ? void 0 : _a.focus(); }} className="flex items-center gap-2 px-2.5 py-2 bg-white/5 hover:bg-white/10 rounded-lg text-white transition-colors text-left">
            ...
            var _a, _b;
            return (<motion.div key={message.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                  {message.type === 'user' && (<motion.div className="flex justify-end" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                  ...
                  {message.type === 'ai' && (<motion.div className="flex justify-start gap-3" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                      ...
                      <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl rounded-tl-md px-5 py-3 max-w-[85%]">
                        {message.content === 'searching_animation' ? (<div className="space-y-4">
                            ...
                            <div className="space-y-2">
                              ...
                              <motion.div className="flex items-center gap-2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }}>
                                <Zap className="w-4 h-4 text-yellow-400"/>
                                <span className="text-gray-300 text-sm">Comparando preços</span>
                              </motion.div>
                              
                              <motion.div className="flex items-center gap-2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }}>
                                <Sparkles className="w-4 h-4 text-purple-400"/>
                          ...
                          </div>) : message.content === 'location_question' ? (<div className="space-y-3">
                            <div className="flex items-center gap-2">
                          ...
                          </div>) : (<p className="text-gray-200 whitespace-pre-wrap text-sm leading-relaxed">{message.content}</p>)}
                      </div>
                    </motion.div>)}

                  {message.type === 'category_question' && (<div className="flex justify-start">
                      <div className="bg-purple-500/10 border border-purple-500/30 rounded-2xl rounded-tl-sm p-4 max-w-lg w-full">
                        ...
                        {((_a = message.categoryQuestion) === null || _a === void 0 ? void 0 : _a.options) && (<div className="grid grid-cols-1 gap-2">
                            {message.categoryQuestion.options.map((option, i) => (<button key={i} onClick={() => handleSend(option)} className="flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 border border-white/20 rounded-lg text-white text-sm text-left transition-colors">
                  ...
                  {message.type === 'image_confirmation' && (<div className="flex justify-start">
                      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.3, type: "spring" }} className="bg-purple-500/10 border border-purple-500/30 rounded-2xl rounded-tl-sm p-4 max-w-lg w-full">
                        <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="flex items-center gap-2 mb-3">
                        ...
                        <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3 mb-2 relative group">
                          ...
                          <input type="text" defaultValue={message.detectedProduct} onClick={(e) => e.stopPropagation()} onKeyDown={(e) => {
                        e.stopPropagation();
                        if (e.key === 'Enter') {
                    ...
                    }} className="w-full bg-transparent text-blue-300 text-sm font-medium outline-none border-b border-blue-500/30 pb-1 focus:border-blue-400 transition-colors cursor-text hover:border-blue-400/50" placeholder="Edite o nome do produto..."/>
                          <p className="text-xs text-gray-400 mt-1">Pressione Enter para salvar</p>
                        </motion.div>
                        
                        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.3 }} className="bg-black/20 rounded-lg p-3 mb-3 border border-white/10">
                          <div className="space-y-2">
                            <div className="flex items-center gap-2">
                        ...
                        <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="flex gap-2">
                          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handleImageSearchReject} disabled={loading || chatState === 'searching'} className="flex-1 px-4 py-2 bg-white/10 border border-white/20 rounded-xl text-white text-sm hover:bg-white/20 transition-colors">
                            Não
                          </motion.button>
                          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handleImagePriceSearch} disabled={loading || chatState === 'searching'} className="flex-1 px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 rounded-xl text-white text-sm disabled:opacity-50 shadow-lg shadow-green-500/20">
                            <motion.span animate={{ opacity: [1, 0.7, 1] }} transition={{ duration: 1.5, repeat: Infinity }}>
                              Sim, buscar!
                            </motion.span>
                          </motion.button>
                        </motion.div>
                  ...
                  {message.type === 'sort_question' && (<div className="flex justify-start">
                      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.3, type: "spring" }} className="bg-blue-500/10 border border-blue-500/30 rounded-2xl rounded-tl-sm p-4 max-w-lg w-full">
                        <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="flex items-center gap-2 mb-3">
                          <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ duration: 0.5, repeat: Infinity, repeatDelay: 2 }}>
                        ...
                        <div className="grid grid-cols-1 gap-2">
                          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => executeImageSearch('BEST_MATCH')} disabled={loading} className="flex items-center gap-3 px-4 py-3 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl text-white text-sm text-left transition-colors disabled:opacity-50">
                          ...
                          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => executeImageSearch('LOWEST_PRICE')} disabled={loading} className="flex items-center gap-3 px-4 py-3 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl text-white text-sm text-left transition-colors disabled:opacity-50">
                          ...
                          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => executeImageSearch('HIGHEST_PRICE')} disabled={loading} className="flex items-center gap-3 px-4 py-3 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl text-white text-sm text-left transition-colors disabled:opacity-50">
                  ...
                  {message.type === 'confirmation' && (<div className="flex justify-start">
                      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.3, type: "spring" }} className="bg-gradient-to-br from-yellow-500/10 to-orange-500/10 border border-yellow-500/30 rounded-3xl rounded-tl-md p-5 max-w-lg w-full shadow-lg">
                        <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="flex items-center gap-2 mb-4">
                          <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ duration: 0.5, repeat: Infinity, repeatDelay: 2 }}>
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.2 }} className="bg-black/30 rounded-2xl p-4 mb-4 border border-white/10 cursor-pointer hover:border-blue-400/50 transition-colors group">
                          <div className="flex items-start gap-3 mb-3">
                            ...
                            <div className="flex-1">
                              ...
                              <input type="text" defaultValue={message.content} onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()} onChange={(e) => {
                        setMessages(prev => {
                            const updated = [...prev];
                            const idx = updated.findIndex(m => m.id === message.id);
                            if (idx >= 0)
                    ...
                    }} className="w-full bg-transparent text-white font-medium text-lg outline-none border-b border-transparent group-hover:border-blue-400/30 focus:border-blue-400 transition-colors pb-1" placeholder="Edite a busca..."/>
                            </div>
                          </div>
                        ...
                        <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }} className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-3 mb-4">
                        ...
                        <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="flex gap-3">
                          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handleCancelSearch} className="flex-1 px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white text-sm hover:bg-white/20 transition-colors font-medium">
                            Cancelar
                          </motion.button>
                          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handleConfirmSearch} disabled={loading || chatState === 'searching'} className="flex-1 px-4 py-3 bg-gradient-to-r from-green-500 to-emerald-600 rounded-xl text-white text-sm disabled:opacity-50 shadow-lg shadow-green-500/30 font-semibold">
                            <motion.span animate={{ opacity: [1, 0.7, 1] }} transition={{ duration: 1.5, repeat: Infinity }} className="flex items-center justify-center gap-2">
                  ...
                  {message.type === 'products' && (<div className="space-y-3 sm:space-y-4">
                      {message.products && message.products.length > 0 && (<>
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3">
                            {message.products.slice(0, 6).map((product, idx) => (<ProductCard key={product.id || idx} product={product}/>))}
                          </div>
                              ...
                              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-black/20 rounded-lg sm:rounded-xl p-3 sm:p-4 mb-3 sm:mb-4 gap-3 sm:gap-0">
                                ...
                                <div className="flex items-center gap-2 sm:gap-3">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-white font-semibold text-xs sm:text-base">{((_b = message.content.match(/Restantes: (\d+)/)) === null || _b === void 0 ? void 0 : _b[1]) || userCredits} créditos</p>
                                  </div>
                                </div>
                              </div>
            ...
            {loading && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-start">
        ...
        <div className="border-t border-white/5 bg-black/20 backdrop-blur-2xl p-4 sm:p-6 safe-area-bottom">
          <div className="max-w-4xl mx-auto">
            <AnimatePresence>
              {uploadedImage && (<motion.div initial={{ opacity: 0, y: 10, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.8, transition: { duration: 0.2 } }} className="mb-3">
                  <div className="relative inline-block">
                    <img src={uploadedImage} alt="Preview" className="h-20 w-20 sm:h-24 sm:w-24 object-cover rounded-2xl border-2 border-blue-500/50 shadow-lg shadow-blue-500/20"/>
                    <motion.button onClick={() => {
                setUploadedImage(null);
                setImageFile(null);
            }} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="absolute -top-2 -right-2 p-1.5 bg-red-500 rounded-full hover:bg-red-600 transition shadow-lg">
            ...
            <div className="flex gap-3">
              ...
              <motion.button onClick={() => { var _a; return (_a = fileInputRef.current) === null || _a === void 0 ? void 0 : _a.click(); }} disabled={loading} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="px-4 py-3.5 bg-white/5 border border-white/10 hover:bg-white/10 rounded-2xl text-white disabled:opacity-50 transition-all flex-shrink-0 shadow-lg" title="Buscar por imagem">
        ...
        }} placeholder={uploadedImage ? "Ou digite..." : "Digite um produto..."} className="flex-1 px-5 py-3.5 bg-white/5 border border-white/10 rounded-2xl text-white placeholder-gray-500 outline-none focus:border-blue-500/50 focus:bg-white/10 text-sm transition-all backdrop-blur-xl" disabled={loading}/>
              
              {uploadedImage ? (<motion.button onClick={handleImageSearch} disabled={loading} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="px-6 py-3.5 bg-gradient-to-r from-purple-600 to-pink-600 rounded-2xl text-white disabled:opacity-50 flex items-center gap-2 shadow-lg shadow-purple-500/30 flex-shrink-0 font-medium">
                  <Search className="w-5 h-5"/>
                  <span className="text-sm hidden sm:inline">Buscar</span>
                </motion.button>) : (<motion.button onClick={() => handleSend()} disabled={loading || !input.trim()} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="px-6 py-3.5 bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl text-white disabled:opacity-50 flex items-center gap-2 shadow-lg shadow-blue-500/30 flex-shrink-0 font-medium">
