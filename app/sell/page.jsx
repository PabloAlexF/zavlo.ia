    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const listingData = {
                ...
                state: (location === null || location === void 0 ? void 0 : location.state) || 'MG',
                city: (location === null || location === void 0 ? void 0 : location.city) || 'Belo Horizonte',
            ...
            if (response.ok) {
                ...
                const errorMsg = Array.isArray(responseData === null || responseData === void 0 ? void 0 : responseData.message)
    ...
    const handleImageUpload = async (e) => {
        ...
        try {
            for (const file of Array.from(files)) {
                ...
                const response = await fetch(`https://api.cloudinary.com/v1_1/${process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME || 'demo'}/image/upload`, { method: 'POST', body: formDataImg });
                if (response.ok) {
                    const data = await response.json();
                    uploadedUrls.push(data.secure_url);
                }
            }
            setFormData(prev => (Object.assign(Object.assign({}, prev), { images: [...prev.images, ...uploadedUrls].slice(0, 10) })));
            ...
            {currentStep === 1 && (<div className="space-y-6">
                ...
                {formData.images.length > 0 && (<div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                    {formData.images.map((img, i) => (<div key={i} className="relative aspect-square rounded-xl overflow-hidden group">
                        ...
                        <button type="button" onClick={() => setFormData(prev => (Object.assign(Object.assign({}, prev), { images: prev.images.filter((_, idx) => idx !== i) })))} className="absolute top-2 right-2 w-6 h-6 bg-red-500 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="text" value={formData.price} onChange={(e) => setFormData(prev => (Object.assign(Object.assign({}, prev), { price: formatPrice(e.target.value) })))} className={`w-full bg-white/10 border rounded-xl pl-12 pr-4 py-3 text-white placeholder-gray-500 focus:ring-2 focus:ring-blue-500/20 transition-all ${formData.price && parseFloat(formData.price.replace(/\./g, '').replace(',', '.')) > 999999.99
                ? 'border-red-500'
                : 'border-white/20 focus:border-blue-500'}`} placeholder="0,00"/>
                      </div>
                      {formData.price && parseFloat(formData.price.replace(/\./g, '').replace(',', '.')) > 999999.99 && (<p className="text-xs text-red-400 mt-1">Valor máximo excedido</p>)}
                    </div>
