...
import { Search, Heart, Tag, Activity, ThumbsUp, Share2, TrendingUp, Bell, Zap, Award, AlertCircle, Image as ImageIcon, Type, CheckCircle, XCircle } from 'lucide-react';
...
export default function DashboardPage() {
        ...
        try {
            ...
            const fetchWithRetry = async (url, options, retries = 3) => {
                for (let i = 0; i < retries; i++) {
                    try {
                        const response = await fetch(url, Object.assign(Object.assign({}, options), { signal: AbortSignal.timeout(30000) }));
                        return response;
                    }
                    catch (error) {
                        if (i === retries - 1)
    ...
    if (loading) {
        return (<div className="min-h-screen bg-gradient-to-br from-[#0B0B0F] via-[#0F0F14] to-[#0B0B0F] flex">
        <DashboardSidebar />
        <MobileSidebar />
    ...
    const getPlanName = (plan) => {
        ...
        return plans[plan === null || plan === void 0 ? void 0 : plan.toLowerCase()] || 'Gratuito';
    };
    // Calcular dados reais para os gráficos
    const salesData = ((_a = metrics === null || metrics === void 0 ? void 0 : metrics.dailyStats) === null || _a === void 0 ? void 0 : _a.map(d => d.searches || 0).slice(-12)) || [45, 52, 48, 65, 58, 72, 68, 75, 82, 78, 88, 95];
    const salesLabels = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    // Dados de usuários ativos (últimos 7 dias de buscas)
    const activeUsersData = ((_b = metrics === null || metrics === void 0 ? void 0 : metrics.dailyStats) === null || _b === void 0 ? void 0 : _b.slice(-7).map(d => d.searches || 0)) || [120, 145, 132, 168, 155, 178, 165];
    ...
    const referralRate = (metrics === null || metrics === void 0 ? void 0 : metrics.topSources) ?
      ...
      <div className="flex-1 lg:ml-64 w-full">
        ...
        <main className="p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 sm:gap-6">
            <StatsCard icon={Search} title="Buscas Hoje" value={(stats === null || stats === void 0 ? void 0 : stats.totalSearches) || 0} trend={searchesTrend} trendLabel="vs ontem" color="from-blue-500 to-cyan-500" delay={0}/>
            <StatsCard icon={Activity} title="Total Mensal" value={((stats === null || stats === void 0 ? void 0 : stats.textSearchesMonth) || 0) + ((stats === null || stats === void 0 ? void 0 : stats.imageSearchesMonth) || 0)} trend={monthlyTrend} trendLabel="este mês" color="from-purple-500 to-pink-500" delay={0.05}/>
            <StatsCard icon={Heart} title="Favoritos" value={(stats === null || stats === void 0 ? void 0 : stats.favoritesCount) || 0} trend={(stats === null || stats === void 0 ? void 0 : stats.favoritesCount) ? 5 : 0} trendLabel="salvos" color="from-red-500 to-orange-500" delay={0.1}/>
            <StatsCard icon={Tag} title="Anúncios" value={(stats === null || stats === void 0 ? void 0 : stats.listingsCount) || 0} trend={(stats === null || stats === void 0 ? void 0 : stats.listingsCount) ? 10 : 0} trendLabel="ativos" color="from-green-500 to-emerald-500" delay={0.15}/>
            <StatsCard icon={AlertCircle} title="Alertas" value={(stats === null || stats === void 0 ? void 0 : stats.alertsCount) || 0} trend={(stats === null || stats === void 0 ? void 0 : stats.alertsCount) ? 8 : 0} trendLabel="monitorando" color="from-yellow-500 to-orange-500" delay={0.2}/>
            <StatsCard icon={Bell} title="Notificações" value={(stats === null || stats === void 0 ? void 0 : stats.notificationsCount) || 0} trend={0} trendLabel="não lidas" color="from-pink-500 to-rose-500" delay={0.25}/>
          </div>

          {/* Credits & Plan Row */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            {/* Credits Card */}
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3, duration: 0.4 }} whileHover={{ y: -5, scale: 1.02 }} className="bg-gradient-to-br from-yellow-500/20 to-orange-500/20 backdrop-blur-xl border border-yellow-500/30 rounded-2xl p-6 relative overflow-hidden group cursor-pointer" onClick={() => router.push('/plans')}>
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-5xl font-black text-yellow-400">{(stats === null || stats === void 0 ? void 0 : stats.credits) || 0}</p>
            ...
            {/* Plan Card */}
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.35, duration: 0.4 }} whileHover={{ y: -5, scale: 1.02 }} className="bg-gradient-to-br from-purple-500/20 to-pink-500/20 backdrop-blur-xl border border-purple-500/30 rounded-2xl p-6 relative overflow-hidden group cursor-pointer" onClick={() => router.push('/plans')}>
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-4xl font-black text-purple-400 capitalize">{(stats === null || stats === void 0 ? void 0 : stats.plan) || 'gratuito'}</p>
                    {planStatus && planStatus.daysLeft !== null && (<p className="text-xs text-purple-300 mt-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            <CircularChart title="Taxa de Sucesso" percentage={satisfactionRate} icon={ThumbsUp} color="from-blue-500 to-purple-500" delay={0.5}/>

            {/* Source Coverage - baseado em top sources */}
            <CircularChart title="Cobertura de Fontes" percentage={referralRate} icon={Share2} color="from-purple-500 to-pink-500" delay={0.6}/>
          </div>

          {/* Charts Row - Dados reais de analytics */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            {/* Sales Overview - Buscas ao longo do tempo */}
            <AreaChart title="Atividade de Buscas" data={salesData} labels={salesLabels} delay={0.7}/>

            {/* Active Users - Buscas dos últimos 7 dias */}
            <BarChart title="Buscas Semanais" data={activeUsersData} labels={activeUsersLabels} delay={0.8}/>
          </div>

          {/* Additional Info Row */}
          {metrics && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9 }} className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/20 rounded-2xl p-6">
              ...
              {/* Top Sources */}
              {Object.keys(metrics.topSources).length > 0 && (<div className="mt-6 pt-6 border-t border-white/10">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    .sort(([, a], [, b]) => b - a)
                    .slice(0, 5)
                    .map(([source, count]) => (<div key={source} className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg">
          ...
          {/* Recent Activity */}
          {history.length > 0 && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.0 }} className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/20 rounded-2xl p-6">
              ...
              <div className="space-y-3">
                {history.map((item, index) => (<motion.div key={index} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0 + index * 0.05 }} className="flex items-center justify-between p-3 bg-white/5 rounded-xl hover:bg-white/10 transition-all group">
